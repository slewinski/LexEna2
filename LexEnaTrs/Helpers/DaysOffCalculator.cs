...
namespace LexEnaTrs.Helpers
{
    public class DaysOffCalculator
    {
        ...
        public static bool IsDayOff(DateTime date)
        {
            ...
            if (date.Month == 05 && date.Day == 03) return true;
        ...
        public static DateTime GetNearestPreviousWorkDay(DateTime changeDate)
        {
            for (int i = 0; i < 14; i++)
            {
                if (DaysOffCalculator.IsDayOff(changeDate))
                {
        ...
        {
            DateTime dt = changeDate;
            dt = dt.AddDays(1);
            for (int i = 0; i < 14; i++)
            {
                if (DaysOffCalculator.IsDayOff(dt))
                {
                    dt = dt.AddDays(1);
                }
                else
