namespace LexEnaTrs.Web
{
    ...
    public class UserRegistrationService : DomainService
    {
        ...
        /// </summary>
        public const string DefaultRole = "Kancelaria";
        ...
        public CreateUserStatus CreateUser(RegistrationData user,
            ...
            [MyRegularExpression("^.*[^a-zA-Z0-9].*$", ErrorMessageResourceName = "ValidationErrorBadPasswordStrength", ErrorMessageResourceType = typeof(ValidationErrorResources))]
            ...
            string password)
        {
            MembershipUser CurrUser;
            CurrUser = Membership.GetUser(false);
            ...
            Uzytkownik lexEnaUser = new Uzytkownik();
            lexEnaUser.UserName = user.UserName;
            lexEnaUser.Nazwisko = user.Nazwisko;
            lexEnaUser.Status = 1; // aktywny
            lexEnaUser.Rola = 0; 
            lexEnaUser.Imie = user.Imie;
            lexEnaUser.JednostkaWindykacji_Id = 1;
            // odczytać z tabeli Uzytkownik  jednoistkę organizacyjną bieżącego użytkownika.

            lexEnaUser.PartitionKey = 1;
            ...
            using (LexEnaMeritumEntities lexena = new LexEnaMeritumEntities())
            {
                try
                {
                    ...
                    lexena.AddToUzytkownik(lexEnaUser);
                    lexena.SaveChanges();
                }
                catch (Exception ex)
                {
